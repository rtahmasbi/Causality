...
Learning about the haplotype $A$ does not improve our ability to predict the outcome $Y$ because the risk in those with ( $A$ = 1) and without ( $A$ = 0) the haplotype is the same, or $Pr[Y = 1|A = 1] = Pr[Y = 1|A = 0]$. In other words, we would intuitively conclude that $A$ and $Y$ are not associated, i.e., $A$ and $Y$ are independent or $A \upvDash Y$.
